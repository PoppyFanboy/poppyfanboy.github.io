...
Это объясняет, почему первый кусок кода должен быть синхронизирован. Ну, а второй должен быть синхронизированным просто потому, что после `satisfyCondition()` оно может обратно ресетнуться.
...
>The Java `volatile` keyword is intended to address variable visibility problems. By declaring the `counter` variable `volatile` all writes to the `counter` variable will be written back to main memory immediately.

Кроме того, если мы делаем запись в `volatile` поле, то это гарантирует, что все переменные, видимые из текущего потока, будут видимы точно таким же образом из других потоков. И, если делаем чтение из `volatile` поля, то все остальные видимые переменные делают re-read. То есть:
...
public class MyClass {
    ...
    // а когда читаем, то сначала надо прочитать из volatile переменной и только потом
    // читать из остальных, потому что чтение из volatile поля триггерит re-read для
    // всех остальных переменных, чтобы удостовериться в их соответствии со своими значениями
    // в кеше или где там
    ...
    // когда делаем запись в volatile поле, то следует *завершать* последовательность
    // апдейтов апдейтом volatile поля, чтобы все остальные переменные тоже апдейтнулись
    // в main memory
...
**Happens-Before Guarantee.** И я подумал, что этого достаточно для того, чтобы тот пример выше с выставлением флага `initialized` работал, если объявить этот флаг как `volatile`. Но ведь компилятор всё ещё может переставить порядок выполнения присваиваний, то есть, например, сначала апдейтнуть `this.days` и только потом все остальные поля. Но, как оказалось, такие штуки запрещены для volatile полей: чтение или запись из `volatile` поля чётко разделяет чтения/записи для других полей. При этом, конечно, что угодно до чтения/записи в `volatile` поле может переставлено между собой. И после тоже.
